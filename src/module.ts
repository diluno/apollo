  ...
  async setup(options, nuxt) {
      ...
      for (let [k, v] of Object.entries(options.clients || {})) {
        if (typeof v === "string") {
          ...
          const resolvedConfig =
            existsSync(path) && (await readConfigFile(path));
        ...
        v.authType =
          (v?.authType === undefined ? options.authType : v?.authType) || null;
        ...
        if (!v?.httpEndpoint && !v?.wsEndpoint) {
          logger.error(
            `Either \`httpEndpoint\` or \`wsEndpoint\` must be provided for client: \`${k}\``
          );
        }
    ...
    nuxt.hook("webpack:config", (configs) => {
      for (const config of configs) {
        // @ts-ignore
        const hasGqlLoader = config.module.rules.some(
          (rule: any) => rule?.use === "graphql-tag/loader"
        );
